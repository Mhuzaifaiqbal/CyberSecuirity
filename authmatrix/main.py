...
class CanAccess:
    ...
    def CanAccess(self):
        flag=0
        if self.op == "" or self.user == "" or self.obj == "":
...
class AddAccess:
    ...
    def AddAcc(self):
        if not self.op or not self.domain or not self.type:
            print("Error: missing operation,domain,type")
            return self.dict

        key = (self.domain, self.type)

        if key not in self.dict:
            self.dict[key] = [self.op]
            print("Success")
        else:
            if isinstance(self.dict[key], list):
                if self.op not in self.dict[key]:
                    self.dict[key].append(self.op)
                    print("Success")
            else:
                if self.dict[key] != self.op:
                    self.dict[key] = [self.dict[key], self.op]
                    print("Success")
